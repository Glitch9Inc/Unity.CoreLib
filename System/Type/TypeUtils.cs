...
namespace Glitch9
{
    ...
    {
        private static readonly Dictionary<Type, TypeCode> k_TypeCodeMap =
            new()
            {
        ...
        private static readonly TypeInformation[] k_TypeCodes =
        {
        ...
        {
            if (k_TypeCodeMap.TryGetValue(t, out TypeCode typeCode))
            {
                isEnum = false;
                return typeCode;
            }
        ...
        {
            TypeInformation typeInformation = k_TypeCodes[(int)convertable.GetTypeCode()];
        ...
        public static object Convert(object initialValue, CultureInfo culture, Type targetType)
        {
            switch (TryConvertInternal(initialValue, culture, targetType, out object? value))
            {
                ...
                case ConvertResult.CannotConvertNull:
                    throw new Exception("Can not convert null {0} into non-nullable {1}.".Format(initialValue.GetType(), targetType));
                case ConvertResult.NotInstantiableType:
                    throw new ArgumentException("Target type {0} is not a value type or a non-abstract class.".Format(targetType), nameof(targetType));
                case ConvertResult.NoValidConversion:
                    throw new InvalidOperationException("Can not convert from {0} to {1}.".Format(initialValue.GetType(), targetType));
                default:
        ...
        private static ConvertResult TryConvertInternal(object initialValue, CultureInfo culture, Type targetType, out object? value)
        {
            ...
            if (IsConvertible(initialValue.GetType()) && IsConvertible(targetType))
            {
                if (targetType.IsEnum)
                {
                    ...
                    else if (initialValue.IsInteger())
                    {
            ...
            if (fromConverter.CanConvertFrom(initialType))
            {
