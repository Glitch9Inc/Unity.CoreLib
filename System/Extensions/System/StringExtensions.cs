...
namespace Glitch9
{
    ...
    {
        private const string TAG = nameof(StringExtensions);

        public static bool LengthCheck(this string text, int minLength, int maxLength)
        {
            if (string.IsNullOrWhiteSpace(text))
            {
                GNLog.Warning(TAG, "Text is null or empty.");
                return false;
            }

            if (text.Length < minLength)
            {
                GNLog.Warning(TAG, $"Text is too short. Min: {minLength}, Current: {text.Length}");
                return false;
            }

            if (text.Length > maxLength)
            {
                GNLog.Warning(TAG, $"Text is too long. Max: {maxLength}, Current: {text.Length}");
                return false;
            }
        ...
        public static int ToInt(this string text)
        {
            ...
            {
                GNLog.Error(TAG, $"Failed to parse int: {text}");
                return 0;
            }
        }
        ...
        public static Vector2 StringToVector2(this string vectorString)
        {
            ...
            {
                GNLog.Error(TAG, $"Failed to parse vector string: {vectorString}");
                return Vector2.zero;
            }
        }
        ...
        public static string[] SafeSplit(this string text, char separator)
        {
            ...
            {
                GNLog.Warning(TAG, $"'{text}' does not contain '{separator}'");
