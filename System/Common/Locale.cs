...
namespace Glitch9
{
    ...
    public readonly struct Locale : IComparable
    {
        ...
        public static explicit operator Locale(SystemLanguage language) => new(language);
        public static explicit operator SystemLanguage(Locale locale) => locale.Value;
    ...
    {
        private static readonly Dictionary<SystemLanguage, string> _cachedDisplayNames = new();
        public static string ToDisplayName(this SystemLanguage language)
        {
            if (_cachedDisplayNames.TryGetValue(language, out string displayName))
            {
                return displayName;
            }
            ...
            _cachedDisplayNames.Add(language, displayName);
            return displayName;
        }
        ...
        {
            return localeCode.ToLower() switch // Ensure the localeCode is in lowercase for comparison
            {
