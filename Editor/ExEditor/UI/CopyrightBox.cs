...
namespace Glitch9
{
    public class CopyrightBox
    {
        ...
        public static void Draw(int year, string productName, string url, string version = null)
        {
            ...
            EditorGUILayout.BeginVertical(ExGUI.Box(10, 10));
            {
                ExGUILayout.Label(new GUIContent($"{productName} version: {version}"), TextAnchor.MiddleCenter, 14);
                ExGUILayout.Label(new GUIContent($"{year} Glitch9 Inc. All Rights Reserved."), TextAnchor.MiddleCenter, 10);
